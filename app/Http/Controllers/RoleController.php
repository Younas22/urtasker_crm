			...
			if (request()->ajax())
			{
				return datatables()->of(
					Auth::user()->role_users_id == 1 
						? Role::latest()->get()  // If admin, fetch all users
						: Role::whereIn('name', ['supperadmin', 'employee', 'Manager', 'HR'])->latest()->get()  // Otherwise, fetch specific roles
				)
					...
					->addColumn('action', function ($data)
					{
						...
						if(Auth::user()->role_users_id == 8){
							if($data->name != 'admin' && $data->name != 'client' && $data->name != 'supperadmin' )
							{
								if (auth()->user()->can('set-permission'))
								{
									$button = '<a class="show btn btn-primary btn-sm mr-1" href="'.route('rolePermission',$data->id).'">' . trans('file.Permission') . '</a>';
						...
						if(Auth::user()->role_users_id == 6){
							if($data->name != 'admin' && $data->name != 'client' && $data->name != 'HR' && $data->name != 'supperadmin' )
							{
								if (auth()->user()->can('set-permission'))
								{
									$button = '<a class="show btn btn-primary btn-sm mr-1" href="'.route('rolePermission',$data->id).'">' . trans('file.Permission') . '</a>';
